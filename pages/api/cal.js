...
export default async (req, res) => {
  ...
  const reqUrl = encodeURI(
    `https://www.googleapis.com/calendar/v3/calendars/${calendarId}/events?key=${API_KEY}&timeMin=${timeMin}`
  );
  ...
  events.forEach((event) => {
    ...
    const body = `Calendar event named ${event.summary} starting at ${format(
      startDate,
      "p 'on' PPP"
    )} and ending at ${format(endDate, "p 'on' PPP")}`;
